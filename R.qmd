...
saveRasterToDisk <- function(time_periods, ssp, res_folder_to_process, path, ext) {
  ...
  for (period in time_periods) {
    ...
    for (scenario in ssp) {
      ...
      res_prefix <- gsub(pattern = "\\.", "_", res_folder_to_process)

      terra::writeRaster(x =terra::rast(result),filename = paste0(output_path, "/", "cmip6_", crop_area_prefix, "_", res_prefix, "_period","_", scenario, ".tiff"))
...
for (period in time_periods) {
  for (scenario in ssp) {
    ...
    write.csv(result,
              file = paste0(output_path, "/", "cmip6_",crop_area_prefix, "_",
