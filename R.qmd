...
ext <-  st_as_sf(vect(here('data', 'input', "extremadura_limites.shp"))) # better use a 3 character var as it is included in the file
...
saveRasterToDisk(time_periods, ssp, res_folder_to_process, path, ext)
```
...
for (period in time_periods) {
  for (scenario in ssp) {
    ...
    for (f in tif_files) print(f) # list files
      ...
      result <- reduceExportToCSV(tif_files, spatial_grid=extremadura, id_column_name="ID1KM") # call function
      print(paste("Writing csv file to disk... Output path: ", output_path))
      write.csv(result,
                file = paste0(output_path, "/", "cmip6_ext_2_5m_",
                                period,"_", scenario, ".csv"))
